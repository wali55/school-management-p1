"use client";

const Announcements = () => {
  return (
    <div className="bg-white w-full rounded-md p-4">
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-semibold">Announcements</h1>
        <span className="text-xs text-gray-400">View More</span>
      </div>
      <div className="flex flex-col gap-4 mt-4">
        <div className="bg-waliSkyLight rounded-md p-4">
          <div className="flex items-center justify-between">
            <h2 className="font-semibold">Lorem ipsum, dolor sit.</h2>
            <span className="text-xs text-gray-400 bg-white rounded-md px-1 py-1">
              2025-02-05
            </span>
          </div>
          <p className="text-sm text-gray-500 mt-1">
            Lorem ipsum, dolor sit amet consectetur adipisicing elit. Magni unde
            illum quae magnam hic? Animi.
          </p>
        </div>
        <div className="bg-waliPurpleLight rounded-md p-4">
          <div className="flex items-center justify-between">
            <h2 className="font-semibold">Lorem ipsum, dolor sit.</h2>
            <span className="text-xs text-gray-400 bg-white rounded-md px-1 py-1">
              2025-02-05
            </span>
          </div>
          <p className="text-sm text-gray-500 mt-1">
            Lorem ipsum, dolor sit amet consectetur adipisicing elit. Magni unde
            illum quae magnam hic? Animi.
          </p>
        </div>
        <div className="bg-waliYellowLight rounded-md p-4">
          <div className="flex items-center justify-between">
            <h2 className="font-semibold">Lorem ipsum, dolor sit.</h2>
            <span className="text-xs text-gray-400 bg-white rounded-md px-1 py-1">
              2025-02-05
            </span>
          </div>
          <p className="text-sm text-gray-500 mt-1">
            Lorem ipsum, dolor sit amet consectetur adipisicing elit. Magni unde
            illum quae magnam hic? Animi.
          </p>
        </div>
      </div>
    </div>
  );
};

export default Announcements;
